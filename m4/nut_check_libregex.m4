    ...
    AS_IF([test -z "${nut_have_libregex_seen}"], [
        ...
        AS_IF([test x"${nut_cv_checked_libregex}" = xyes], [
            ...
            AS_IF([test "${nut_noncv_checked_libregex_now}" = no], [
                ...
                AC_MSG_NOTICE([libregex (cached): CFLAGS=${LIBREGEX_CFLAGS}])
                AC_MSG_NOTICE([libregex (cached): LIBS=${LIBREGEX_LIBS}])
                AC_MSG_NOTICE([libregex (cached): module=${LIBREGEX_MODULE} requires_source=${LIBREGEX_REQUIRES_SOURCE} REQUIRES=${LIBREGEX_REQUIRES}])
            ])
        ])
    ])
])
