...
if test -z "${nut_ax_realpath_lib_prereq_seen}"; then
    ...
    AS_CASE(["${target_os}"],
        [*mingw*], [
            ...
            AC_CHECK_TARGET_TOOLS([LD], [ld ld.exe], [false])
        ]
    )
fi
])
...
[
    ...
    AS_IF([test "x$GCC" = xyes -o "x$CLANGCC" = xyes], [
        ...
        dnl # Primarily we care to know dynamically linked (shared object) files
        AS_CASE(["${myLIBNAME}"],
            [*.so|*.a|*.o|*.lo|*.la|*.dll|*.dll.a], [],
            [
                AS_CASE(["${target_os}"],
        ...
        AS_CASE(["${target_os}"],
            [*mingw*], [
                AS_IF([test x"${LD}" != x -a x"${LD}" != xfalse -a x"${DLLTOOL}" != x -a x"${DLLTOOL}" != xfalse], [
                    AS_CASE(["${myLIBNAME}"],
                        [lib*], [
                            myLIBNAME_LD="`echo "$myLIBNAME" | sed -e 's/^lib/-l/' -e sed 's/\.\(dll\|dll\.a\|a\)$//i'`"
